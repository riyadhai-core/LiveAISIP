...
//!
//! An Unsupported field contains a non-empty ordered comma-separated list of
//! SIP option tags that are not supported by the sender of the response.
//!
...
//! rules governing 420 responses and correlation with `Require` or
//! `Proxy-Require` belong to SIP message validation and transaction logic.
...
/// Maximum accepted SIP `Unsupported` field-value size in bytes.
///
/// This is a `LiveAISIP` operational limit rather than a SIP protocol limit.
pub const MAX_UNSUPPORTED_BYTES: usize = 8 * 1024;

/// Maximum number of option tags accepted in one `Unsupported` field value.
pub const MAX_UNSUPPORTED_OPTION_TAGS: usize = 64;

/// A validated SIP `Unsupported` field value.
///
...
impl Unsupported {
    /// Creates an Unsupported value containing one option tag.
    #[must_use]
    ...
    /// Creates an Unsupported value from a non-empty ordered option-tag vector.
    ///
    /// # Errors
    ///
    ...
    pub fn from_option_tags(option_tags: Vec<OptionTag>) -> Result<Self, ParseError> {
        ...
        if option_tags.len() > MAX_UNSUPPORTED_OPTION_TAGS {
            return Err(ParseError::TooManyOptionTags {
                maximum: MAX_UNSUPPORTED_OPTION_TAGS,
            });
        }

        let length = serialized_length(&option_tags);

        if length > MAX_UNSUPPORTED_BYTES {
            return Err(ParseError::TooLong {
                length,
                maximum: MAX_UNSUPPORTED_BYTES,
            });
        }
    ...
    /// Parses a SIP `Unsupported` field value from wire bytes.
    ///
    ...
    /// Returns all unsupported option tags in wire order.
    #[must_use]
    pub fn option_tags(&self) -> &[OptionTag] {
        &self.option_tags
    }

    /// Returns mutable access to all unsupported option tags.
    #[must_use]
    pub fn option_tags_mut(&mut self) -> &mut [OptionTag] {
        &mut self.option_tags
    }

    /// Returns the first unsupported option tag.
    ///
    /// Successfully constructed Unsupported values are always non-empty.
    #[must_use]
    pub fn first(&self) -> &OptionTag {
        &self.option_tags[0]
    }

    /// Returns the number of unsupported option tags.
    #[must_use]
    pub fn len(&self) -> usize {
        self.option_tags.len()
    }

    /// Returns whether the unsupported option-tag list is empty.
    ///
    /// Successfully constructed Unsupported values are never empty.
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.option_tags.is_empty()
    }

    /// Returns whether the specified option tag is listed as unsupported.
    ///
    ...
    /// Returns whether an option tag with this textual name is unsupported.
    ///
    /// Comparison follows SIP token semantics and is ASCII case-insensitive.
    #[must_use]
    pub fn has_option_tag(&self, option_tag: &str) -> bool {
        self.option_tags
            .iter()
            .any(|candidate| candidate.as_str().eq_ignore_ascii_case(option_tag))
    }

    /// Appends another unsupported option tag while preserving ordering.
    ///
    ...
    pub fn push(&mut self, option_tag: OptionTag) -> Result<(), ParseError> {
        if self.option_tags.len() >= MAX_UNSUPPORTED_OPTION_TAGS {
            return Err(ParseError::TooManyOptionTags {
                maximum: MAX_UNSUPPORTED_OPTION_TAGS,
            });
        }
...
/// Parses a SIP `Unsupported` field value.
///
/// # Errors
///
/// Returns [`ParseError`] when the field value violates Unsupported syntax or
/// an operational bound.
pub fn parse(input: &[u8]) -> Result<Unsupported, ParseError> {
    if input.len() > MAX_UNSUPPORTED_BYTES {
        return Err(ParseError::TooLong {
            length: input.len(),
            maximum: MAX_UNSUPPORTED_BYTES,
        });
    ...
    Unsupported::from_option_tags(option_tags)
}
...
/// Failure to parse or construct a SIP `Unsupported` field value.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[non_exhaustive]
pub enum ParseError {
    /// The Unsupported field value was empty.
    Empty,
    ...
    /// One unsupported option tag was malformed.
    InvalidOptionTag {
        /// Zero-based option-tag position.
        option_index: usize,
    ...
    /// The field exceeded the bounded unsupported option-tag count.
    TooManyOptionTags {
        /// Maximum accepted unsupported option-tag count.
        maximum: usize,
    },
}
...
impl fmt::Display for ParseError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Empty => formatter.write_str("SIP Unsupported field value is empty"),
            Self::TooLong { length, maximum } => {
                write!(
                    formatter,
                    "SIP Unsupported field-value length {length} exceeds maximum {maximum}"
                )
            }
            Self::InvalidLineBreak => {
                formatter.write_str("SIP Unsupported contains an invalid line break")
            }
            Self::EmptyOptionTag { option_index } => {
                write!(
                    formatter,
                    "SIP Unsupported option tag at position {option_index} is empty"
                )
            }
            Self::InvalidOptionTag {
                ...
                write!(
                    formatter,
                    "invalid SIP Unsupported option tag at position {option_index}: {source}"
                )
            }
            Self::TooManyOptionTags { maximum } => {
                write!(
                    formatter,
                    "SIP Unsupported contains more than {maximum} option tags"
                )
            }
        }
    }
...
mod tests {
    use super::{
        MAX_UNSUPPORTED_BYTES, MAX_UNSUPPORTED_OPTION_TAGS, ParseError, Unsupported, parse,
    };
    use crate::sip::headers::supported::{OptionTag, OptionTagError};
    ...
    fn parses_single_option_tag() {
        let Ok(unsupported) = parse(b"100rel") else {
            panic!("expected valid Unsupported value");
        };

        assert_eq!(unsupported.len(), 1);
        assert!(unsupported.first().is_100rel());
    }

    #[test]
    fn parses_multiple_option_tags() {
        let Ok(unsupported) = parse(b"100rel, precondition, timer, replaces") else {
            panic!("expected valid Unsupported value");
        };

        assert_eq!(unsupported.len(), 4);
        assert!(unsupported.option_tags()[0].is_100rel());
        assert_eq!(unsupported.option_tags()[1].as_str(), "precondition");
        assert!(unsupported.option_tags()[2].is_timer());
        assert_eq!(unsupported.option_tags()[3].as_str(), "replaces");
    }

    #[test]
    fn option_tags_compare_case_insensitively() {
        let Ok(unsupported) = parse(b"100REL, TIMER, PreCondition") else {
        ...
        assert!(unsupported.has_option_tag("precondition"));
        assert!(unsupported.has_option_tag("PRECONDITION"));
    }

    #[test]
    fn known_option_tags_are_canonicalized() {
        let Ok(unsupported) = parse(b"100REL, TIMER, PATH, OUTBOUND, GRUU") else {
            panic!("expected known option tags");
        };

        assert_eq!(
            unsupported.to_string(),
            "100rel, timer, path, outbound, gruu"
        );
    }

    #[test]
    fn extension_option_tag_spelling_is_preserved() {
        let Ok(unsupported) = parse(b"X-LiveAISIP.Option") else {
            panic!("expected extension option tag");
        };

        assert_eq!(unsupported.first().as_str(), "X-LiveAISIP.Option");
        assert!(unsupported.first().is_extension());
    }

    #[test]
    fn extension_lookup_is_case_insensitive() {
        let Ok(unsupported) = parse(b"X-Custom") else {
            panic!("expected extension option tag");
        };

        assert!(unsupported.has_option_tag("X-Custom"));
        assert!(unsupported.has_option_tag("x-custom"));
        assert!(unsupported.has_option_tag("X-CUSTOM"));
    }
    ...
    fn contains_uses_option_tag_semantics() {
        let Ok(unsupported) = parse(b"TIMER, X-Custom") else {
            panic!("expected valid Unsupported value");
        };

        assert!(unsupported.contains(&OptionTag::timer()));

        let Ok(extension) = OptionTag::new("x-custom") else {
            panic!("expected extension option tag");
        };

        assert!(unsupported.contains(&extension));
    }

    #[test]
    fn preserves_option_tag_order() {
        let Ok(unsupported) = parse(b"precondition, 100rel, timer, replaces") else {
            panic!("expected ordered option tags");
        };

        assert_eq!(unsupported.option_tags()[0].as_str(), "precondition");
        assert!(unsupported.option_tags()[1].is_100rel());
        assert!(unsupported.option_tags()[2].is_timer());
        assert_eq!(unsupported.option_tags()[3].as_str(), "replaces");
    }
    ...
    fn repeated_option_tags_are_preserved() {
        let Ok(unsupported) = parse(b"timer, TIMER, 100rel") else {
            panic!("expected repeated option tags");
        };

        assert_eq!(unsupported.len(), 3);
        assert!(unsupported.option_tags()[0].is_timer());
        assert!(unsupported.option_tags()[1].is_timer());
        assert!(unsupported.option_tags()[2].is_100rel());
    }

    #[test]
    fn accepts_whitespace_around_commas() {
        let Ok(unsupported) = parse(b" \t100rel\t,\tprecondition  ,   timer\t ") else {
            panic!("expected delimiter whitespace");
        };

        assert_eq!(unsupported.to_string(), "100rel, precondition, timer");
    }
    ...
    fn rejects_empty_constructor_vector() {
        assert_eq!(
            Unsupported::from_option_tags(Vec::new()),
    ...
    fn rejects_field_above_size_limit() {
        let input = vec![b'a'; MAX_UNSUPPORTED_BYTES + 1];

        assert_eq!(
            parse(&input),
            Err(ParseError::TooLong {
                length: MAX_UNSUPPORTED_BYTES + 1,
                maximum: MAX_UNSUPPORTED_BYTES,
            })
        );
    }
    ...
    fn constructor_creates_single_option_tag() {
        let unsupported = Unsupported::new(OptionTag::rel100());
    ...
    fn constructs_from_multiple_option_tags() {
        ...
        let Ok(unsupported) = Unsupported::from_option_tags(option_tags) else {
            panic!("expected valid option-tag vector");
        };

        assert_eq!(unsupported.to_string(), "100rel, timer, path");
    }

    #[test]
    fn push_appends_option_tag() {
        let mut unsupported = Unsupported::new(OptionTag::rel100());

        assert!(unsupported.push(OptionTag::timer()).is_ok());
        assert!(unsupported.push(OptionTag::path()).is_ok());
    ...
    fn push_allows_duplicate_option_tag() {
        let mut unsupported = Unsupported::new(OptionTag::timer());
    ...
    fn rejects_too_many_option_tags_during_construction() {
        let option_tags = (0..=MAX_UNSUPPORTED_OPTION_TAGS)
    ...
    fn rejects_too_many_option_tags_during_parsing() {
        let input = std::iter::repeat_n("a", MAX_UNSUPPORTED_OPTION_TAGS + 1)
            .collect::<Vec<_>>()
            .join(",");
    ...
    fn push_enforces_option_tag_count() {
        ...
        let Ok(mut unsupported) = Unsupported::from_option_tags(option_tags) else {
            panic!("expected option-tag list at operational limit");
        };

        assert_eq!(
            unsupported.push(OptionTag::path()),
            Err(ParseError::TooManyOptionTags {
                maximum: MAX_UNSUPPORTED_OPTION_TAGS,
            })
        );
    ...
    #[test]
    fn parses_from_str() {
        let Ok(unsupported) = Unsupported::from_str("100rel, precondition, timer") else {
            panic!("expected valid Unsupported value");
        };
    ...
    fn empty_string_from_str_is_rejected() {
        assert_eq!(Unsupported::from_str(""), Err(ParseError::Empty));
    }

    #[test]
    fn consumes_into_option_tags() {
        let Ok(unsupported) = parse(b"100rel, timer") else {
            panic!("expected valid Unsupported value");
        };

        let option_tags = unsupported.into_option_tags();
    ...
    #[test]
    fn unsupported_equality_uses_option_tag_semantics() {
        let Ok(first) = parse(b"TIMER, X-Custom") else {
            panic!("expected valid first Unsupported value");
        };

        let Ok(second) = parse(b"timer, x-custom") else {
            panic!("expected valid second Unsupported value");
        };
