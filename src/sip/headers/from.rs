...
//! `addr-spec`, semicolon-delimited parameters are interpreted as `From`
//! header parameters.
//!
...
/// Maximum accepted SIP `From` field-value size in bytes.
///
/// This is a `LiveAISIP` operational limit rather than a SIP protocol limit.
pub const MAX_FROM_BYTES: usize = 8 * 1024;

/// Maximum number of parameters accepted on one `From` field value.
pub const MAX_FROM_PARAMETERS: usize = 64;

/// Maximum accepted `tag` value size in bytes.
///
/// SIP does not prescribe this operational ceiling.
pub const MAX_FROM_TAG_BYTES: usize = 256;

/// Maximum accepted extension parameter-name size in bytes.
pub const MAX_FROM_PARAMETER_NAME_BYTES: usize = 256;

/// Maximum accepted extension parameter-value size in bytes.
pub const MAX_FROM_PARAMETER_VALUE_BYTES: usize = 1024;

/// A validated SIP `From` field value.
...
impl FromHeader {
    /// Creates a `From` value without a `tag` or extension parameters.
    #[must_use]
    ...
    /// Parses a SIP `From` field value from wire bytes.
    ///
    ...
    /// Returns the address carried by this `From` value.
    #[must_use]
    ...
    pub fn set_tag(&mut self, tag: impl Into<Box<str>>) -> Result<(), ParseError> {
        ...
        if self.tag.is_none() && self.parameter_count() >= MAX_FROM_PARAMETERS {
            return Err(ParseError::TooManyParameters {
                maximum: MAX_FROM_PARAMETERS,
            });
        }
    ...
    #[must_use]
    pub fn parameters(&self) -> &[FromParameter] {
        &self.parameters
    }
    ...
    #[must_use]
    pub fn parameter(&self, name: &str) -> Option<&FromParameter> {
        self.parameters
            .iter()
    ...
    /// parameter must be managed through [`FromHeader::set_tag`].
    ///
    /// # Errors
    ///
    ...
    /// has been reached.
    pub fn push_parameter(&mut self, parameter: FromParameter) -> Result<(), ParseError> {
        if self.parameter_count() >= MAX_FROM_PARAMETERS {
            return Err(ParseError::TooManyParameters {
                maximum: MAX_FROM_PARAMETERS,
            });
        }
    ...
    /// Returns the total number of `From` header parameters, including `tag`.
    #[must_use]
    ...
    #[must_use]
    pub fn into_parts(self) -> (Address, Option<Box<str>>, Vec<FromParameter>) {
        (self.address, self.tag, self.parameters)
    }
}

impl fmt::Display for FromHeader {
...
/// A validated generic `From` header parameter.
///
/// The reserved `tag` parameter is not representable through this type and is
/// stored separately by [`FromHeader`].
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct FromParameter {
    name: Box<str>,
...
impl fmt::Display for FromParameter {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
...
/// Parses a SIP `From` field value.
///
/// # Errors
///
/// Returns [`ParseError`] when the field value violates SIP syntax or an
/// operational bound.
pub fn parse(input: &[u8]) -> Result<FromHeader, ParseError> {
    if input.len() > MAX_FROM_BYTES {
        return Err(ParseError::TooLong {
            length: input.len(),
            maximum: MAX_FROM_BYTES,
        });
    }
    ...
    let mut header = FromHeader::new(address);
...
fn parse_parameters(header: &mut FromHeader, mut input: &[u8]) -> Result<(), ParseError> {
    loop {
        ...
        if header.parameter_count() >= MAX_FROM_PARAMETERS {
            return Err(ParseError::TooManyParameters {
                maximum: MAX_FROM_PARAMETERS,
            });
        }
...
fn parse_parameter_name(input: &[u8]) -> Result<(&str, &[u8]), ParseError> {
    ...
    if end > MAX_FROM_PARAMETER_NAME_BYTES {
        return Err(ParseError::ParameterNameTooLong {
            length: end,
            maximum: MAX_FROM_PARAMETER_NAME_BYTES,
        });
    }
...
fn parse_extension_parameter<'a>(
    name: &str,
    input: &'a [u8],
) -> Result<(FromParameter, &'a [u8]), ParseError> {
    validate_extension_parameter_name(name.as_bytes())?;

    let input = trim_lws_start(input);

    if input.is_empty() || input[0] == b';' {
        return Ok((FromParameter::flag(name)?, input));
    }
...
fn parse_unquoted_extension_parameter<'a>(
    ...
    let parameter = FromParameter::unquoted(name, value)?;
...
fn parse_quoted_extension_parameter<'a>(
    ...
    let parameter = FromParameter::quoted(name, value)?;
...
fn validate_tag(input: &[u8]) -> Result<(), ParseError> {
    ...
    if input.len() > MAX_FROM_TAG_BYTES {
        return Err(ParseError::TagTooLong {
            length: input.len(),
            maximum: MAX_FROM_TAG_BYTES,
        });
    }
...
fn validate_extension_parameter_name(input: &[u8]) -> Result<(), ParseError> {
    ...
    if input.len() > MAX_FROM_PARAMETER_NAME_BYTES {
        return Err(ParseError::ParameterNameTooLong {
            length: input.len(),
            maximum: MAX_FROM_PARAMETER_NAME_BYTES,
        });
...
fn validate_unquoted_parameter_value(input: &[u8]) -> Result<(), ParseError> {
    ...
    if input.len() > MAX_FROM_PARAMETER_VALUE_BYTES {
        return Err(ParseError::ParameterValueTooLong {
            length: input.len(),
            maximum: MAX_FROM_PARAMETER_VALUE_BYTES,
        });
    }
...
/// Failure to parse or construct a SIP `From` value.
#[derive(Clone, Debug, Eq, PartialEq)]
#[non_exhaustive]
pub enum ParseError {
...
impl fmt::Display for ParseError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Empty => formatter.write_str("SIP From field value is empty"),
            Self::TooLong { length, maximum } => {
                write!(
                    formatter,
                    "SIP From field-value length {length} exceeds maximum {maximum}"
                )
            }
            Self::MissingAddress => formatter.write_str("SIP From address is missing"),
            Self::InvalidAddress(error) => {
                write!(formatter, "invalid SIP From address: {error}")
            }
            Self::InvalidAddressStructure => {
                formatter.write_str("SIP From name-addr structure is invalid")
            }
            Self::MissingClosingAngle => {
                formatter.write_str("SIP From name-addr is missing its closing angle bracket")
            }
            Self::BareUriRequiresNameAddr { index, byte } => {
                write!(
                    formatter,
                    "SIP From bare URI contains byte 0x{byte:02x} at offset {index} requiring name-addr form"
                )
            }
            Self::InvalidQuotedString => formatter.write_str("SIP From quoted string is invalid"),
            Self::UnexpectedTrailingData => {
                formatter.write_str("unexpected data follows SIP From field content")
            }
            Self::EmptyParameter => formatter.write_str("SIP From parameter is empty"),
            Self::InvalidParameterName { index, byte } => {
                write!(
                    formatter,
                    "invalid SIP From parameter-name byte 0x{byte:02x} at offset {index}"
                )
            }
            Self::ParameterNameTooLong { length, maximum } => {
                write!(
                    formatter,
                    "SIP From parameter-name length {length} exceeds maximum {maximum}"
                )
            }
            Self::ReservedParameterName => {
                formatter.write_str("SIP From tag must use the dedicated tag parameter")
            }
            Self::InvalidParameterSeparator { byte } => {
                write!(
                    formatter,
                    "invalid SIP From parameter separator byte 0x{byte:02x}"
                )
            }
            Self::MissingParameterValue => {
                formatter.write_str("SIP From parameter value is missing")
            }
            Self::InvalidParameterValue { index, byte } => {
                write!(
                    formatter,
                    "invalid SIP From parameter-value byte 0x{byte:02x} at offset {index}"
                )
            }
            Self::ParameterValueTooLong { length, maximum } => {
                write!(
                    formatter,
                    "SIP From parameter-value length {length} exceeds maximum {maximum}"
                )
            }
            Self::MissingTagValue => formatter.write_str("SIP From tag value is missing"),
            Self::InvalidTag { index, byte } => {
                write!(
                    formatter,
                    "invalid SIP From tag byte 0x{byte:02x} at offset {index}"
                )
            }
            Self::TagTooLong { length, maximum } => {
                write!(
                    formatter,
                    "SIP From tag length {length} exceeds maximum {maximum}"
                )
            }
            Self::DuplicateParameter => {
                formatter.write_str("SIP From parameter name is duplicated")
            }
            Self::TooManyParameters { maximum } => {
                write!(
                    formatter,
                    "SIP From contains more than {maximum} parameters"
                )
            }
        }
    }
...
mod tests {
    use super::{
        FromHeader, FromParameter, MAX_FROM_BYTES, MAX_FROM_PARAMETER_NAME_BYTES,
        MAX_FROM_PARAMETER_VALUE_BYTES, MAX_FROM_PARAMETERS, MAX_FROM_TAG_BYTES, ParseError, parse,
    };
    use crate::sip::parser::address;
    use crate::sip::types::address::Address;
    ...
    fn parses_basic_name_addr() {
        let Ok(header) = parse(b"<sip:alice@example.com>") else {
            panic!("expected valid From value");
        };
    ...
    fn parses_name_addr_with_tag() {
        let Ok(header) = parse(b"<sip:alice@example.com>;tag=1928301774") else {
            panic!("expected valid From value");
        };

        assert_eq!(header.tag(), Some("1928301774"));
        assert_eq!(header.parameter_count(), 1);
    }

    #[test]
    fn parses_display_name_and_tag() {
        let Ok(header) = parse(b"\"Alice Smith\" <sip:alice@example.com>;tag=a84b4c") else {
            panic!("expected valid From value");
        };

        assert_eq!(header.address().display_name(), Some("Alice Smith"));
        assert_eq!(header.tag(), Some("a84b4c"));
    }

    #[test]
    fn tag_name_is_case_insensitive() {
        let Ok(header) = parse(b"<sip:alice@example.com>;TAG=abc123") else {
    ...
    fn tag_value_case_is_preserved() {
        ...
        assert_eq!(header.tag(), Some("AbC123"));
        assert_eq!(header.to_string(), "<sip:alice@example.com>;tag=AbC123");
    }

    #[test]
    fn parses_bare_addr_spec_with_tag() {
        let Ok(header) = parse(b"sip:alice@example.com;tag=887s6pa") else {
            panic!("expected valid bare From address");
        };

        assert!(header.address().is_addr_spec());
        assert_eq!(header.address().uri().to_string(), "sip:alice@example.com");
        assert_eq!(header.tag(), Some("887s6pa"));
    }

    #[test]
    fn bare_semicolon_parameter_is_header_parameter() {
        let Ok(header) = parse(b"sip:alice@example.com;transport=tcp") else {
            panic!("expected generic From parameter");
        };
        ...
        let Some(parameter) = header.parameter("transport") else {
            panic!("expected From header parameter");
        };
    ...
    fn bracketed_semicolon_parameter_remains_uri_parameter() {
        let Ok(header) = parse(b"<sip:alice@example.com;transport=tcp>;tag=abc") else {
    ...
    fn parses_flag_extension_parameter() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-feature") else {
    ...
    fn parses_token_extension_parameter() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-mode=fast") else {
    ...
    fn parses_quoted_extension_parameter() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-label=\"Voice Gateway\"") else {
    ...
    fn quoted_parameter_may_contain_semicolon() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-label=\"one;two\";tag=abc") else {
            panic!("expected quoted semicolon");
        };

        assert_eq!(
            header.parameter("x-label").and_then(FromParameter::value),
            Some("one;two")
        );
        assert_eq!(header.tag(), Some("abc"));
    }
    ...
    fn quoted_parameter_unescapes_quote() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-label=\"Alice \\\"Voice\\\"\"") else {
    ...
    fn quoted_parameter_unescapes_backslash() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-path=\"one\\\\two\"") else {
            panic!("expected escaped backslash");
        };

        assert_eq!(
            header.parameter("x-path").and_then(FromParameter::value),
            Some("one\\two")
        );
    }
    ...
    fn parses_ipv6_reference_generic_value() {
        let Ok(header) = parse(b"<sip:alice@example.com>;x-host=[2001:db8::1]") else {
            panic!("expected IPv6-reference generic value");
        };

        assert_eq!(
            header.parameter("x-host").and_then(FromParameter::value),
            Some("[2001:db8::1]")
        );
    }
    ...
    fn preserves_extension_parameter_order() {
        let Ok(header) = parse(b"<sip:alice@example.com>;first=1;second=2;third=3") else {
    ...
    fn allows_whitespace_around_parameter_separators() {
        let Ok(header) = parse(b"<sip:alice@example.com> ; tag = abc ; x-mode = fast") else {
            panic!("expected separator whitespace");
        };

        assert_eq!(header.tag(), Some("abc"));
        assert_eq!(
            header.parameter("x-mode").and_then(FromParameter::value),
            Some("fast")
        );
    }
    ...
    fn trims_surrounding_field_whitespace() {
        let Ok(header) = parse(b" \t<sip:alice@example.com>;tag=abc \t") else {
    ...
    fn rejects_field_above_size_limit() {
        let input = vec![b'A'; MAX_FROM_BYTES + 1];

        assert_eq!(
            parse(&input),
            Err(ParseError::TooLong {
                length: MAX_FROM_BYTES + 1,
                maximum: MAX_FROM_BYTES,
            })
        );
    }
    ...
    fn rejects_bare_uri_with_comma() {
        assert_eq!(
            parse(b"sip:alice@example.com,other"),
            Err(ParseError::BareUriRequiresNameAddr {
                index: 21,
                byte: b',',
            })
    ...
    fn rejects_missing_closing_angle() {
        assert_eq!(
            parse(b"<sip:alice@example.com;tag=abc"),
    ...
    fn rejects_empty_parameter_between_semicolons() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;x=1;;y=2"),
    ...
    fn rejects_tag_without_equal_sign() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;tag"),
    ...
    fn rejects_invalid_tag_value() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;tag=bad:value"),
    ...
    fn rejects_duplicate_tag() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;tag=one;TAG=two"),
    ...
    fn rejects_empty_unquoted_parameter_value() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;x-mode="),
    ...
    fn rejects_invalid_unquoted_parameter_value() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;x-mode=bad:value"),
    ...
    fn rejects_unterminated_quoted_parameter_value() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;x-label=\"unfinished"),
            Err(ParseError::InvalidQuotedString)
        );
    }
    ...
    fn rejects_crlf_in_quoted_parameter_value() {
        assert_eq!(
            parse(b"<sip:alice@example.com>;x-label=\"one\r\ntwo\""),
            Err(ParseError::InvalidQuotedString)
        );
    }
    ...
    fn creates_flag_extension_parameter() {
        let Ok(parameter) = FromParameter::flag("x-feature") else {
    ...
    fn creates_unquoted_extension_parameter() {
        let Ok(parameter) = FromParameter::unquoted("x-mode", "fast") else {
    ...
    fn creates_quoted_extension_parameter() {
        let Ok(parameter) = FromParameter::quoted("x-label", "Voice Gateway") else {
    ...
    fn quoted_parameter_serialization_escapes_special_characters() {
        let Ok(parameter) = FromParameter::quoted("x-label", "A \"B\" \\ C") else {
            panic!("expected valid quoted parameter");
        };
    ...
    fn rejects_parameter_name_above_size_limit() {
        let name = "A".repeat(MAX_FROM_PARAMETER_NAME_BYTES + 1);

        assert_eq!(
            FromParameter::flag(name),
            Err(ParseError::ParameterNameTooLong {
                length: MAX_FROM_PARAMETER_NAME_BYTES + 1,
                maximum: MAX_FROM_PARAMETER_NAME_BYTES,
            })
        );
    }
    ...
    fn rejects_parameter_value_above_size_limit() {
        let value = "A".repeat(MAX_FROM_PARAMETER_VALUE_BYTES + 1);

        assert_eq!(
            FromParameter::unquoted("x-value", value),
            Err(ParseError::ParameterValueTooLong {
                length: MAX_FROM_PARAMETER_VALUE_BYTES + 1,
                maximum: MAX_FROM_PARAMETER_VALUE_BYTES,
            })
        );
    }
    ...
    fn rejects_tag_above_size_limit() {
        ...
        let mut header = FromHeader::new(address);
        let tag = "A".repeat(MAX_FROM_TAG_BYTES + 1);

        assert_eq!(
            header.set_tag(tag),
            Err(ParseError::TagTooLong {
                length: MAX_FROM_TAG_BYTES + 1,
                maximum: MAX_FROM_TAG_BYTES,
            })
        );
    }
    ...
    fn enforces_total_parameter_count() {
        ...
        for index in 0..MAX_FROM_PARAMETERS {
            let name = format!("x-{index}");
            let Ok(parameter) = FromParameter::flag(name) else {
        ...
        let Ok(extra) = FromParameter::flag("x-extra") else {
            panic!("expected valid extension parameter");
        };
    ...
    fn tag_counts_toward_total_parameter_limit() {
        ...
        for index in 0..MAX_FROM_PARAMETERS - 1 {
            let name = format!("x-{index}");
            let Ok(parameter) = FromParameter::flag(name) else {
                panic!("expected valid extension parameter");
            };
    ...
    fn display_serializes_canonical_value() {
        let Ok(header) = parse(b"\"Alice\" <sip:alice@example.com>;TAG=AbC;x-mode=\"fast mode\"")
        else {
            panic!("expected valid From value");
        };
        ...
        assert_eq!(
            header.to_string(),
            "\"Alice\" <sip:alice@example.com>;tag=AbC;x-mode=\"fast mode\""
        );
    }
