...
use super::manager::{ManagerConfig, ManagerError, TransportManager};
use super::tcp_driver::{TcpDriver, TcpDriverError, WriteProgress};
use super::tls_driver::{TlsDriver, TlsDriverError, TlsShutdownProgress, TlsWriteProgress};
use super::udp::{OutboundDatagram, UdpConfig, UdpError};
use super::udp_driver::{UdpDriver, UdpDriverError};
...
impl ReliableDriver {
    ...
    const fn peer_addr(&self) -> std::net::SocketAddr {
        match self {
    ...
    fn wants_socket_write(&self) -> bool {
        match self {
            Self::Tcp(driver) => driver[0].pending_write_bytes() != 0,
            Self::Tls(driver) => {
    ...
    fn try_clone_socket(&self) -> io::Result<TcpStream> {
        match self {
    ...
    fn flush_transport(&mut self) -> Result<bool, ServiceError> {
        match self {
            Self::Tcp(_) => Ok(true),
    ...
    fn start_send(&mut self, message: Arc<[u8]>) -> Result<bool, ServiceError> {
        match self {
    ...
    fn flush_send(&mut self) -> Result<bool, ServiceError> {
        match self {
            Self::Tcp(driver) => driver[0]
                .flush_send()
                .map(|progress| matches!(progress, WriteProgress::Complete))
    ...
    fn receive(&mut self) -> Result<ReceivedMessage, ServiceError> {
        match self {
    ...
    fn shutdown(&mut self) -> Result<ServiceShutdownProgress, ServiceError> {
        match self {
            Self::Tcp(driver) => driver[0]
                .shutdown()
...
fn flow_id(id: ConnectionId) -> Result<FlowId, ServiceError> {
    FlowId::new(id.get()).map_err(|_| ServiceError::InternalConnectionInvariant)
}

fn box_one<T>(value: T) -> Result<Box<[T; 1]>, ServiceError> {
