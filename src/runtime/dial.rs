...
impl OutboundDialConfig {
    ...
    /// Installs a complete bounded SDP body.
    ///
    /// # Errors
    ///
    ...
    pub fn prepare(
        ...
        match self.body {
            ...
            InviteBody::InactivePcmu => {
                let body = inactive_pcmu_sdp(advertised_addr)?;
                invite = invite.with_sdp(&body).map_err(OutboundDialError::Invite)?;
            }
        }
        signaling
...
fn inactive_pcmu_sdp(address: SocketAddr) -> Result<Box<[u8]>, OutboundDialError> {
    ...
    let address_type = if address.is_ipv4() { "IP4" } else { "IP6" };
    Ok(format!(
